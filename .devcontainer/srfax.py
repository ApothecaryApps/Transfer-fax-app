...
def queue_fax(secrets, to_fax_number: str, pdf_bytes: bytes, filename: str = "transfer.pdf"):
    """Queue a PDF fax. to_fax_number must be 11 digits, e.g. 16025551212."""
    ...
    return str(data["Result"])  # FaxDetailsID
...
def wait_for_fax(secrets, fax_details_id: str, timeout_sec: int = 300, poll_every: int = 10):
    """Poll until Sent/Failed or timeout. Returns final status payload."""
    deadline = time.time() + timeout_sec
    last = None
    while time.time() < deadline:
        last = get_fax_status(secrets, fax_details_id)
        # Result can be dict-like depending on SRFax JSON shape
